#!/usr/bin/env node
// Smoke test for /aftersales/crm/nps-dashboard（售後 NPS 看板）
// - 純 dashboard，不做 CRUD
...
const log = (...m) => console.error("[aftersales-nps-dashboard-smoke]", ...m);
...
async function main() {
  ...
  // 1) 載入 dashboard
  {
    const resp = await page.goto(`${BASE}/aftersales/crm/nps-dashboard`, {
  ...
  // 2) H1
  {
    ...
    results.push(
      h1.includes("售後 NPS 看板") || h1.includes("售後NPS看板")
        ? { ok: true, step: "H1" }
  ...
  // 7) 批評者清單
  {
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `aftersales-nps-dashboard-default-${uniqSuffix}.png`),
      fullPage: true,
    });
  }

  // 8) 切 range = 7d → URL 帶 ?range=7d 且仍在 aftersales path
  {
    await page.getByTestId("nps-range-select").selectOption("7d");
    await page
      .waitForURL(
        (u) => u.toString().includes("/aftersales/crm/nps-dashboard") && u.toString().includes("range=7d"),
        { timeout: NAV_TIMEOUT },
      )
      .catch(() => {});
    const cur = page.url();
    const ok = cur.includes("/aftersales/crm/nps-dashboard") && cur.includes("range=7d");
    results.push(
      ok
        ? { ok: true, step: "range=7d switch" }
        : { ok: false, step: "range=7d switch", reason: cur },
    );
  }
  ...
  // 9) 切 range = all → URL 帶 ?range=all
  {
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `aftersales-nps-dashboard-all-${uniqSuffix}.png`),
      fullPage: true,
    });
  }
...
main().catch((e) => {
  console.error("[aftersales-nps-dashboard-smoke] unexpected", e);
  process.exit(2);
});
