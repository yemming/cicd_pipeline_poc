#!/usr/bin/env node
// Smoke test for /sales/crm/call-tasks （銷售電訪工作檯）
...
const log = (...m) => console.error("[call-tasks-smoke]", ...m);
...
async function main() {
  ...
  const newNotes = `煙霧測試備註 ${uniqSuffix}`;
  const updatedNotes = `煙霧測試備註更新 ${uniqSuffix}`;

  // 1) 載入 list
  {
    const resp = await page.goto(
      `${BASE}/sales/crm/call-tasks?kind=sales`,
  ...
  // 2) H1
  {
    ...
    results.push(
      h1.includes("銷售電訪") && h1.includes("工作檯")
  ...
  // 6) +新增 link → /new
  {
    ...
    const okHref = href === "/sales/crm/call-tasks/new?kind=sales";
    results.push(
      okHref
    ...
    await page
      .waitForURL(
        (target) => new URL(target).pathname === "/sales/crm/call-tasks/new",
        { timeout: NAV_TIMEOUT },
      )
      .catch(() => {});
    const pathname = new URL(page.url()).pathname;
    results.push(
      pathname === "/sales/crm/call-tasks/new"
  ...
  // 7) Create mode UI
  {
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `sales-call-tasks-create-${uniqSuffix}.png`),
      fullPage: true,
    });
  }
  ...
  // 8) 選客戶 + 建立
  {
    ...
    await page
      .waitForURL(
        (target) =>
          /^\/sales\/crm\/call-tasks\/[0-9a-f-]{36}$/.test(
    ...
    const m = /^\/sales\/crm\/call-tasks\/([0-9a-f-]{36})$/.exec(path2);
    if (m) {
      createdId = m[1];
      results.push({
        ok: true,
  ...
  if (createdId) {
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `sales-call-tasks-detail-${uniqSuffix}.png`),
      fullPage: true,
    });
  }
  ...
  if (createdId) {
    ...
    if (banner.includes("已儲存") || banner.includes("✓")) {
      ...
      const stillDetail = /^\/sales\/crm\/call-tasks\/[0-9a-f-]{36}$/.test(
  ...
  if (createdId) {
    // reload detail to read attempt count
    await page.goto(`${BASE}/sales/crm/call-tasks/${createdId}`, {
    ...
    if (startBtnCount === 0) {
      ...
      await page
        .locator(
          'div[role="status"]:has-text("已開始撥打"), div[role="status"]:has-text("✓")',
        )
        .first()
  ...
  if (createdId) {
    ...
    await page
      ...
      .waitFor({ timeout: 15_000 })
      .catch(() => {});
    await page.getByTestId("call-task-notes-input").fill(updatedNotes);
    await page.locator('button:has-text("儲存變更")').first().click();
    await page
      .locator('button:has-text("儲存中…")')
      .waitFor({ state: "detached", timeout: 30_000 })
  ...
  // 14) 回 list 看新筆出現
  if (createdId) {
    await page.goto(`${BASE}/sales/crm/call-tasks?kind=sales`, {
      waitUntil: "domcontentloaded",
      timeout: NAV_TIMEOUT,
    });
    const linkCount = await page
      .locator(`a[href="/sales/crm/call-tasks/${createdId}"]`)
      .count();
    results.push(
      linkCount > 0
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `sales-call-tasks-list-${uniqSuffix}.png`),
      fullPage: true,
    });
  }
  ...
  if (createdId) {
    ...
    await page
      .waitForURL(
        (target) => new URL(target).pathname === "/sales/crm/call-tasks",
        { timeout: NAV_TIMEOUT },
      )
      .catch(() => {});
    results.push(
      new URL(page.url()).pathname === "/sales/crm/call-tasks"
  ...
  // 16) console errors（忽略 hydration / caret-color noise）
