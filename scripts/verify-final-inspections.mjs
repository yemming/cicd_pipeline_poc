...
const SHOTS = path.join(__dirname, '..', 'tmp', 'verify-final-inspections');
...
(async () => {
  ...
  try {
    const resp = await page.goto(`${BASE}/parts/aftersales/final-inspections`, { waitUntil: 'networkidle' });
    result.steps.push({ status: resp.status(), final_url: page.url() });
    if (resp.status() !== 200) throw new Error(`HTTP ${resp.status()}`);

    await page.waitForSelector('h1:has-text("竣工複檢")', { timeout: 8000 });
    ...
    const statusHits =
      (await page.locator('text=待複檢').count()) +
      (await page.locator('text=已通過').count()) +
      (await page.locator('text=複檢中').count()) +
      (await page.locator('text=未通過').count()) +
