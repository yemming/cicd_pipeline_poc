...
const FLAT = [
  ...
  "/parts/operations/exceptions/new",
  "/parts/operations/exceptions?type=exception_in",
  "/parts/operations/exceptions?status=posted",
...
async function main() {
  ...
  try {
    await page.goto(`${BASE}/parts/operations/exceptions`, { waitUntil: "domcontentloaded", timeout: NAV_TIMEOUT });
    const hrefs = await page.locator('a[href^="/parts/operations/exceptions/"]').evaluateAll((els) =>
      els.map((e) => e.getAttribute("href")).filter(Boolean),
    );
    const detail = hrefs.find((h) => /^\/parts\/operations\/exceptions\/[0-9a-f-]{36}$/.test(h));
    if (detail) {
      const r = await check(page, detail);
      log(r.ok ? `✓ ${detail} (detail)` : `✗ ${detail} — ${r.reason}`);
      results.push(r);
    } else {
      log(`⊘ exceptions/[id] — list 空、跳過`);
    }
  } catch (e) {
