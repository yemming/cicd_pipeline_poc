#!/usr/bin/env node
// Smoke test for /aftersales/crm/push-notifications（推播通知設定 - 售後）
...
const log = (...m) => console.error("[aftersales-push-smoke]", ...m);
...
async function main() {
  ...
  // 1) Load page
  {
    const resp = await page.goto(`${BASE}/aftersales/crm/push-notifications`, {
  ...
  // 2b) Sprint chip = AS-CRM06（找 main 內、含 h1 那個 header）
  {
    const txt = await page
      ...
      .catch(() => "");
    results.push(
      txt.includes("AS-CRM06")
        ? { ok: true, step: "sprint chip AS-CRM06" }
        : { ok: false, step: "sprint chip AS-CRM06", reason: txt.slice(0, 200) },
    );
  }

  // 3) Summary chip shows 6 events
  {
  ...
  // 4) Event cards count == 6
  {
    ...
    await page.screenshot({
      path: path.join(TMP_DIR, `aftersales-push-default-${uniqSuffix}.png`),
      fullPage: true,
    });
  }

  // 4b) Check aftersales-specific event codes show up
  {
    const codes = await page
      .getByTestId("push-event-card")
      .evaluateAll((els) => els.map((e) => e.getAttribute("data-event-code")));
    const expected = [
    ...
    results.push(
      missing.length === 0
        ? { ok: true, step: "aftersales event codes present", codes }
        : { ok: false, step: "aftersales event codes present", reason: `missing=${missing.join(",")}` },
    );
  }
  ...
  // 7) Reload → 狀態保留
  {
    await page.goto(`${BASE}/aftersales/crm/push-notifications`, {
      waitUntil: "domcontentloaded",
      timeout: NAV_TIMEOUT,
    });
    const v = await page
  ...
  // 8) Cleanup — toggle back to original
  {
    ...
    await page
      .screenshot({
        path: path.join(TMP_DIR, `aftersales-push-after-${uniqSuffix}.png`),
        fullPage: true,
      })
      .catch(() => {});
  }
...
main().catch((e) => {
  console.error("[aftersales-push-smoke] unexpected", e);
  process.exit(2);
});
