...
const FLAT = ["/parts/issue/internal-sale", "/parts/issue/internal-sale/new"];
...
async function main() {
  ...
  try {
    await page.goto(`${BASE}/parts/issue/internal-sale`, { waitUntil: "domcontentloaded", timeout: NAV_TIMEOUT });
    const hrefs = await page.locator('a[href^="/parts/issue/internal-sale/"]').evaluateAll((els) =>
      els.map((e) => e.getAttribute("href")).filter(Boolean),
    );
    const detail = hrefs.find((h) => /^\/parts\/issue\/internal-sale\/[0-9a-f-]{36}$/.test(h));
    if (detail) {
      const r = await check(page, detail);
      log(r.ok ? `✓ ${detail} (detail)` : `✗ ${detail} — ${r.reason}`);
      results.push(r);
    } else {
      log(`⊘ internal-sale/[id] — list 空、跳過`);
    }
  } catch (e) {
