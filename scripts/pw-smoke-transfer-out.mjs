...
const FLAT = ["/parts/issue/transfer-out", "/parts/issue/transfer-out/new"];
...
async function main() {
  ...
  try {
    await page.goto(`${BASE}/parts/issue/transfer-out`, { waitUntil: "domcontentloaded", timeout: NAV_TIMEOUT });
    const hrefs = await page.locator('a[href^="/parts/issue/transfer-out/"]').evaluateAll((els) =>
      els.map((e) => e.getAttribute("href")).filter(Boolean),
    );
    const detail = hrefs.find((h) => /^\/parts\/issue\/transfer-out\/[0-9a-f-]{36}$/.test(h));
    if (detail) {
      const r = await check(page, detail);
      log(r.ok ? `✓ ${detail} (detail)` : `✗ ${detail} — ${r.reason}`);
      results.push(r);
    } else {
      log(`⊘ transfer-out/[id] — list 空、跳過`);
    }
  } catch (e) {
