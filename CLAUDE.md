...
│                                                              │
│  客戶提需求                                                   │
│  /feedback/tickets/new ──→ Supabase: feedback_tickets        │
│                                       status: draft          │
│                                          │                   │
│                             [Admin 審批，手動]                │
│                                          │                   │
...
│                    ┌─────────────────────┘                   │
│                    │  Phase A（現在）：手動                   │
│                    │  管理員手動 trigger GitHub Actions       │
│                    │                                         │
│                    │  Phase B（未來）：自動                   │
│                    │  Supabase webhook → GitHub Actions      │
│                    │  → Claude Code API → auto PR            │
│                    └─────────────────────┘                   │
│                                          │                   │
│                                  GitHub PR created           │
│                                          │                   │
...
**現階段（Phase A，手動 CI/CD）**：
1. 用戶在 `/feedback/tickets/new` 提交需求
2. Admin 在詳情頁將狀態改為 `in_progress`（只有 admin 有權限）
3. Admin 手動確認需求 → 手動 trigger GitHub Actions（或直接開發）
4. 開發完成 → PR → merge → Zeabur 自動部署
5. 部署後 Admin 將狀態改為 `released`

**下一步（Phase B，自動化）**：
- Supabase Database Webhook：`feedback_tickets.status = in_progress` 時觸發
- GitHub Actions 接收 webhook payload（含 ticket title + description）
- Claude Code API 讀取需求 → 自動生成 PR
- PR 由人工審批後 merge（不跳過人工最終確認這一關）

### 前端結構

| 路由 | 元件 | 功能 |
|------|------|------|
| `/feedback/tickets` | `tickets/page.tsx` + `TicketsBoard` | 列表 + 看板視圖（status 欄位拖曳） |
| `/feedback/tickets/new` | `tickets/new/page.tsx` + `TicketForm` | 建立許願單（title / url / description） |
| `/feedback/tickets/[id]` | `tickets/[id]/page.tsx` | 詳情：狀態機 + `CommentThread` + `CanvasPanel` |

**元件位置**：`src/components/feedback/`

| 元件 | 職責 |
|------|------|
| `ticket-card.tsx` | 卡片（標籤 chip / 投票數 / 更新時間） |
| `ticket-form.tsx` | 建立/編輯表單 |
| `status-actions.tsx` | 狀態按鈕（Admin-only 限制在 server action 層） |
| `comment-thread.tsx` | 留言時間軸 + 新增留言表單 |
| `canvas-panel.tsx` | Lazy-load 包裝（避免 SSR 問題） |
| `canvas-panel-impl.tsx` | Excalidraw 實作 + snapshot upsert 到 Supabase |
| `tickets-board.tsx` | 看板視圖（四欄：draft / in_progress / review / released） |

**Server Actions**（`src/lib/feedback-actions.ts`）：
- `createTicket(fd)` — 建立許願單
- `updateTicketStatus(ticketId, next)` — 狀態轉移（含 admin 驗證）
- `addComment(ticketId, body)` — 新增留言
- `saveCanvasSnapshot(ticketId, snapshot)` — 畫布存檔
