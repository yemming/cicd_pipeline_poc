  ...
  return (
    <main className="px-6 py-5 space-y-3">
      <header className="flex items-center gap-2.5">
        ...
        <span className="text-[12px] text-[#9A9890]">接收來自其他倉庫的調撥單、確認到貨</span>
      </header>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-[#EEECE6] hover:bg-[#F8F7F4]">
                      ...
                      <td className="px-3 py-2 font-mono text-[12px]">{fmtDate(r.expected_arrival_date)}</td>
      ...
      {!canEdit && (
        <div className="text-[11px] text-[#9A9890]">💡 你目前沒有入庫操作權限，僅能檢視</div>
      )}
    </main>
  );
}
