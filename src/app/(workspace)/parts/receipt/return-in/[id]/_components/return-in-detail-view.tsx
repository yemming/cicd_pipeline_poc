...
export function ReturnInDetailView({
  receipt,
  canEdit,
}: {
        ...
        <div className="ml-auto flex items-center gap-1.5">
          {mode === "view" ? (
            <>
              ...
              <span
                title="Phase 2 開放"
                className="h-[30px] px-4 rounded-full text-[12px] font-medium inline-flex items-center bg-[#0F6E56] text-white opacity-50 cursor-not-allowed shadow-sm"
              >
                ＋ 新增退料入庫
              </span>
              <button
      ...
      <header className="bg-white border border-[#EEECE6] rounded-lg p-4">
        <div className="flex items-stretch gap-4">
          ...
          <div className="shrink-0 w-[260px] h-[120px] bg-[#F8F7F4] border border-[#EEECE6] rounded-lg flex flex-col items-center justify-center gap-1">
            <div className="text-[11px] text-[#9A9890]">退貨總金額</div>
      ...
      <section className="bg-white border border-[#EEECE6] rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-3">
          ...
          <Kv
            ...
            value={
              receipt.source_gi_no ? (
                <span className="font-mono">{receipt.source_gi_no}</span>
              ) : (
                "—"
              )
            }
        ...
        >
          ...
          >
            <header className="px-4 py-3 border-b border-[#EEECE6]">
              <h3 className="text-[14px] font-semibold text-[#2C2C2A]">作廢退料入庫單</h3>
            </header>
            <div className="px-4 py-4 space-y-3">
              <p className="text-[12.5px] text-[#5A5955] leading-relaxed">
                作廢後將自動沖回 <b>{lineCount} 筆退入庫存</b>。
                若任一庫存已被消耗，無法作廢。
              </p>
              <div>
      ...
      <table className="w-full text-[12px] border-collapse">
        <thead>
          <tr className="border-b border-[#EEECE6] bg-[#F8F7F4]">
            ...
            <th className="text-left px-2 py-2 text-[11px] text-[#9A9890] font-medium w-[100px]">退入倉位</th>
            <th className="text-right px-2 py-2 text-[11px] text-[#9A9890] font-medium w-[80px]">退入數</th>
