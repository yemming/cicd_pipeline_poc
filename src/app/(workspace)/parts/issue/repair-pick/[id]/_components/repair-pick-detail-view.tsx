...
export function RepairPickDetailView({
  issue,
  canEdit,
}: {
    ...
    <main className={`px-6 py-5 space-y-3 ${isPending ? "pointer-events-none opacity-60" : ""}`}>
      ...
      <header className="bg-white border border-[#EEECE6] rounded-lg p-4">
        <div className="flex items-stretch gap-4">
          <div className="flex-1 min-w-0 flex flex-col gap-2">
            <div>
              <div className="text-[11px] tracking-wider text-[#9A9890]">維修領料單</div>
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap text-[12px]">
                ...
                {issue.customer_name ? (
                  <span className="text-[#5A5955]">{issue.customer_name}</span>
                ) : null}
          ...
          <div className="shrink-0 w-[260px] h-[120px] bg-[#F8F7F4] border border-[#EEECE6] rounded-lg flex flex-col items-center justify-center gap-1">
            <div className="text-[11px] text-[#9A9890]">出庫總金額</div>
      ...
      <section className="bg-white border border-[#EEECE6] rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-3">
          <Kv label="領料單號" value={issue.gi_no} mono />
          <Kv
            label="狀態"
            value={
              <span
          ...
          <Kv
            ...
            value={
              issue.ro_no ? (
                <span className="font-mono text-[#185FA5]">{issue.ro_no}</span>
              ) : (
                <span className="text-[11px] text-[#9A9890]">ad-hoc（不綁工單）</span>
              )
            }
          />
          <Kv label="車主" value={issue.customer_name ?? "—"} />
        ...
        >
          ...
          >
            <header className="px-4 py-3 border-b border-[#EEECE6]">
              <h3 className="text-[14px] font-semibold text-[#2C2C2A]">作廢領料單</h3>
            </header>
