  ...
  if (!(await hasPermission(PERMISSIONS.ISSUE_VIEW))) {
    return (
      <main className="px-6 py-6">
        <p className="text-[14px] text-[#BF2600]">沒有檢視內售出庫的權限</p>
      </main>
    );
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12.5px]">
            <thead className="bg-[#F4F4F4] text-[#444]">
              <tr>
                <th className="px-3 py-2 text-left">出庫單號</th>
                <th className="px-3 py-2 text-left">收貨方</th>
                <th className="px-3 py-2 text-left">出庫倉</th>
                <th className="px-3 py-2 text-left">出庫日</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="px-3 py-2">{r.customer_label ?? "—"}</td>
