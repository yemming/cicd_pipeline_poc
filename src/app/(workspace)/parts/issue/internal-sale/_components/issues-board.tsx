  ...
  return (
    <main className="px-6 py-5 space-y-3">
      <header className="flex items-center gap-2.5">
        ...
        <span className="text-[12px] text-[#9A9890]">{subtitle}</span>
      </header>
      ...
      <section className="bg-white border border-[#EEECE6] rounded-lg px-4 py-3">
        <div className="flex gap-2 items-end flex-wrap">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-[#9A9890] font-medium">出庫單號搜尋</label>
            <input
              type="text"
              value={q}
              onChange={(e) => setQ(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && applyFilter()}
              placeholder="搜尋 GI 編號..."
      ...
      <div className="flex items-center gap-2">
        <span className="text-[12px] text-[#9A9890]">
          共 <b className="text-[#2C2C2A]">{rows.length}</b> 筆出庫
        </span>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F8F7F4]">
                <th className="px-3 py-2 text-left text-[11px] text-[#9A9890] font-semibold">出庫單號</th>
                <th className="px-3 py-2 text-left text-[11px] text-[#9A9890] font-semibold">出庫倉</th>
                <th className="px-3 py-2 text-left text-[11px] text-[#9A9890] font-semibold">出庫日期</th>
                <th className="px-3 py-2 text-right text-[11px] text-[#9A9890] font-semibold">出庫總數</th>
                <th className="px-3 py-2 text-right text-[11px] text-[#9A9890] font-semibold">出庫金額</th>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-[#EEECE6] hover:bg-[#F8F7F4]">
                      ...
                      <td className="px-3 py-2 font-mono text-[12px]">{fmtDate(r.issue_date)}</td>
                      <td className="px-3 py-2 text-right font-mono text-[12px]">
                        {r.qty_issued_total ?? 0}
                      </td>
                      <td className="px-3 py-2 text-right font-mono text-[12px]">
