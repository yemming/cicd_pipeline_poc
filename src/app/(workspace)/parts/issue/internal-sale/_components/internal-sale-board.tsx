      ...
      },
      {
        ...
        cell: (r) => (
          <span className="text-[12.5px]">
            {r.customer_name ?? <span className="text-[#9A9890]">（未指定）</span>}
          </span>
        ),
  ...
  return (
    <main className={`px-6 py-5 space-y-3 ${isPending ? "pointer-events-none opacity-60" : ""}`}>
      ...
      <div className="flex items-center gap-3">
        <span className="text-[12px] text-[#9A9890]">
          共 <b className="text-[#2C2C2A]">{rows.length}</b> 筆出貨單
        </span>
        <span className="text-[12px] text-[#9A9890]">
          結算總額 <b className="text-[#2C2C2A] font-mono">{fmtMoney(totalAmount)}</b>
        </span>
      </div>
      ...
      <DataGrid
        ...
        exportFileName="internal-sales"
        emptyMessage="沒有符合條件的內售單"
        disabled={isPending}
        rowActionsWidth={170}
        rowActions={(r) => (
          <>
