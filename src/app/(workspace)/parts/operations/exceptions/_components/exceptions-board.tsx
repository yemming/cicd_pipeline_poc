...
export function ExceptionsBoard({
  ...
  function applyFilter() {
    ...
    if (q) params.set("q", q);
    startTransition(() =>
      router.push(`/parts/operations/exceptions${params.toString() ? "?" + params : ""}`),
    );
  }
  ...
  return (
    <main className="px-6 py-5 space-y-3">
      <header className="flex items-center gap-2.5">
        ...
        <span className="text-[12px] text-[#9A9890]">非標準流程的庫存調整：盤點 / 損耗 / 例外進出</span>
      </header>
      ...
      <section className="bg-white border border-[#EEECE6] rounded-lg px-4 py-3">
        <div className="flex gap-2 items-end flex-wrap">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-[#9A9890] font-medium">調整單號</label>
            <input
              type="text"
              value={q}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F8F7F4]">
                <th className="px-3 py-2 text-left text-[11px] text-[#9A9890] font-semibold">調整單號</th>
                ...
                <th className="px-3 py-2 text-right text-[11px] text-[#9A9890] font-semibold">影響金額</th>
                <th className="px-3 py-2 text-left text-[11px] text-[#9A9890] font-semibold">建立日</th>
                ...
                rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-[#EEECE6] hover:bg-[#F8F7F4]">
                      ...
                      <td className="px-3 py-2 text-[12px]">{TYPE_LABEL[r.type ?? ""] ?? r.type ?? "—"}</td>
                      ...
                      <td className="px-3 py-2 font-mono text-[12px]">{fmtDate(r.created_at)}</td>
