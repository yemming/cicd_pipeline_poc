...
/**
 * AI 拍名片 — 拍照 / 上傳圖檔 → Gemini Vision 抽 9 欄 → review 後建客戶
 *
...
import { BusinessCardReview } from "./business-card-review";
import { BusinessCardHistoryItem } from "./business-card-history-item";

type Phase = "idle" | "captured" | "uploading" | "result" | "error";

export function BusinessCardApp({
  recent,
}: {
  recent: BusinessCardScanListItem[];
}) {
  ...
  const [historicReviewed, setHistoricReviewed] = useState<
    BusinessCardScanListItem["reviewed_values"] | null
  >(null);
  const [dragOver, setDragOver] = useState(false);

  const [localRecent, setLocalRecent] =
    useState<BusinessCardScanListItem[]>(recent);
  useEffect(() => {
  ...
  function loadHistoric(item: BusinessCardScanListItem) {
    // 從最近紀錄載入：用既有 scanId 重新 render review
    setResult({
      scanId: item.id,
  ...
  async function uploadAndScan() {
    ...
    try {
      const r = await scanBusinessCard(fd);
      if (!r.ok) {
        setErrMsg(r.error);
        setPhase("error");
        return;
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="text-[12px] text-[#5A5955] leading-relaxed">
                  Gemini 5-15 秒抽出姓名 / 公司 / 職稱 / 電話 / Email / 地址
                </div>
              </div>
