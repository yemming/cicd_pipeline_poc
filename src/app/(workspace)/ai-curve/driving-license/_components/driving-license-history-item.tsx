...
/**
 * 最近紀錄單筆（駕照版）— 左滑刪除 + 桌機 ✕ fallback
 */

import { useRef, useState, useTransition } from "react";
import {
  deleteDrivingLicenseScan,
...
export function DrivingLicenseHistoryItem({
  item,
  onLoad,
  onDeleted,
}: {
  item: DrivingLicenseScanListItem;
  onLoad: (item: DrivingLicenseScanListItem) => void;
  ...
  const aiClass = item.ai_suggestions?.license_class?.value || "";
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const r = await deleteDrivingLicenseScan(item.id);
      if (r.ok) {
        onDeleted(item.id);
      } else {
        setErrMsg(r.error);
