...
function actionChipClass(action: string): string {
  if (action.includes("approved") || action.includes("passed") || action.includes("complete")) return "bg-[#EAF3DE] text-[#3B6D11]";
  if (action.includes("rejected") || action.includes("delete") || action.includes("cancel") || action.includes("cleared")) return "bg-[#FDECEA] text-[#CC0000]";
  if (action.includes("discount") || action.includes("requested") || action.includes("update")) return "bg-[#FDF3E3] text-[#854F0B]";
  if (action.includes("status_changed") || action.includes("create")) return "bg-[#EAF4FB] text-[#185FA5]";
...
const columns: DataGridColumn<AuditLogRow>[] = [
  ...
  },
  {
    ...
    width: 90,
    cell: (r) => (
      <span className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[11px] whitespace-nowrap ${r.brand_id === "ducati" ? "bg-[#FDECEA] text-[#CC0000]" : "bg-[#EAF4FB] text-[#185FA5]"}`}>
        {r.brand_id ?? "—"}
      </span>
    ),
...
export function GroupAuditBoard({
  ...
  function handleReset() {
    ...
    setLocalDateFrom("");
    setLocalDateTo("");
    pushParams({ brand_id: "", action: "", table_name: "", date_from: "", date_to: "" });
  }
      ...
      <section className="bg-white border border-[#EEECE6] rounded-lg px-4 py-3">
        <div className="flex gap-2 items-end flex-wrap">
          <div className="flex flex-col gap-1">
            ...
            <select className={inputClass} value={localBrand} onChange={(e) => setLocalBrand(e.target.value)}>
              {BRAND_OPTIONS.map((o) => (
