        ...
        <div className="flex items-center gap-4 mb-4 px-1">
          <div className="shrink-0 flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <div className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.25em] text-[#CC0000]">
              <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ backgroundColor: "#CC0000" }} />
              Ducati Taipei
            </div>
            <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#CC0000]">Official Dealer</span>
          </div>
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                <div
                  ...
                  style={{ backgroundColor: m.accent ? `${m.accent}22` : "#CC000022" }}
                >
                  <span className="material-symbols-outlined text-2xl" style={{ color: m.accent ?? "#CC0000" }}>
                    {m.icon}
                  </span>
                </div>
...
function StatBubble({ label, value, accent, icon }: { label: string; value: string; accent: string; icon: string }) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: `${accent}18`, color: accent }}>
        <span className="material-symbols-outlined text-[20px]">{icon}</span>
      </div>
      <div className="flex flex-col leading-tight min-w-0">
        <div className="text-xl font-extrabold font-display leading-none tabular-nums" style={{ color: accent }}>{value}</div>
        <div className="text-[10px] text-slate-500 font-semibold tracking-wide whitespace-nowrap mt-0.5">{label}</div>
      </div>
    </button>
  );
}
