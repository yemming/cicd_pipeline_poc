...
import type { AppointmentListRow, AppointmentLookups } from "@/domain/appointments.constants";

type Mode = "view" | "edit" | "create";
type Banner = { ok: boolean; msg: string } | null;

/** 此元件掛在 /service/appointments/* 路徑下，basePath 指向 service 路由 */
const LIST_HREF = "/service/appointments";

export function ServiceAppointmentDetailView({
  appointment,
  lookups,
  canEdit,
  initialMode,
  ...
  useSetPageHeader({
    title: appointment
      ? `預約 ${(appointment.appointment_time as string).slice(0, 5)} ${appointment.customer_name ?? ""}`
      : "新增預約",
    breadcrumb: [
      { label: "維修管理", href: "/service" },
      { label: "預約看板", href: LIST_HREF },
      ...
      <header className="bg-white border border-[#EEECE6] rounded-lg p-4">
        <div className="flex items-stretch gap-4">
          <div className="flex-1 min-w-0 flex flex-col gap-2">
            <div>
              <div className="text-[11px] tracking-wider text-[#9A9890]">售後預約 · 維修管理</div>
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap text-[12px]">
                {appointment ? (
                  <>
                    <span className="font-mono text-[#5A5955]">{appointment.appointment_date}</span>
                    <span
        ...
        <div className="px-4 py-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, customer_id: e.target.value || null, vehicle_id: null })
              }
            >
        ...
        <div className="px-4 py-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, service_subtype: e.target.value || null })
              }
            >
              <option value="">—</option>
