...
export const SEARCH_REGISTRY: SearchTableSpec[] = [
  ...
  {
    entityType: "ro_checkout",
    label: "結帳單",
    icon: "point_of_sale",
    color: "#1A3A5C",
    description: "結帳單號(CK)",
    table: "ro_checkouts",
    searchFields: ["checkout_no"],
    selectColumns: "id, checkout_no, status, updated_at",
    entryHref: "/parts/aftersales/checkout",
    toHit: (r) => ({
      entity_type: "ro_checkout",
      entity_id: String(r.id),
      title: String(r.checkout_no ?? "(未編號結帳單)"),
      subtitle: (r.status as string | null) ?? null,
